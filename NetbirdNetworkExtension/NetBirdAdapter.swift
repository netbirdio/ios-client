...
public class NetBirdAdapter {
    ...
    init?(with tunnelManager: PacketTunnelProviderSettingsManager, configPath: String? = nil, statePath: String? = nil) {
        ...
        guard let client = NetBirdSDKNewClient(resolvedConfigPath, resolvedStatePath, Preferences.cacheDirectory(), deviceName, osVersion, osName, self.networkChangeListener, self.dnsManager) else {
